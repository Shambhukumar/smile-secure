...
import { teathNameAndNumber } from "../TeathNameAndNumber";
import "./dentalmap.scss";
const DentelMap = () => {

	const {selectedTeath, setSelectedTeath,teathPositionInfo, setTeachPostionInfo} = useContext(SelectedToothContext);
 
  useEffect(() => {
	if(submitDisbale && Object.values(selectedTeath).length){
  ...
  const [submitDisbale, setSubmitDisable] = useState(true);
  ...
  const handleSelect = (e) => {
    ...
    if (!selectedTeath[e]) {
      setSelectedTeath((prevState) => {
        return { ...prevState, [e]: teathNameAndNumber[e] };
      });
	  if(submitDisbale){
		setSubmitDisable(false)
	  }
      ...
      const obj = { ...selectedTeath };
      delete obj[e];
	  if(!Object.values(obj).length){
		setSubmitDisable(true)
	}
      setSelectedTeath(obj);
      let isModthere = 0;
      let isquadrantthere = 0;
      let isPositionthere = 0;
      Object.values(obj).forEach((element)=>{
              if(element.mod === teathNameAndNumber[e].mod){
                  isModthere = ++isModthere
              }
              if(element.quadrant === teathNameAndNumber[e].quadrant){
                isquadrantthere = ++isquadrantthere
              }
              if(element.position === teathNameAndNumber[e].position){
                isPositionthere = ++isPositionthere
              }
    ...
    //   console.log("founded: "+isModthere )
      ...
      if(isquadrantthere === 0){
        setTeachPostionInfo(prevstate=>{
            return {...prevstate, [teathNameAndNumber[e].quadrant]: false}
        })
    }
      if(isPositionthere === 0){
        setTeachPostionInfo(prevstate=>{
            return {...prevstate, [teathNameAndNumber[e].position]: false}
        })
    }
    }
    ...
    if(!teathPositionInfo[teathNameAndNumber[e].quadrant]){
        setTeachPostionInfo(prevstate=>{
            return {...prevstate, [teathNameAndNumber[e].quadrant]: true}
        })
    }
    if(!teathPositionInfo[teathNameAndNumber[e].position]){
        setTeachPostionInfo(prevstate=>{
            return {...prevstate, [teathNameAndNumber[e].position]: true}
        })
    }
  ...
  return (
    <div className="chart">
      ...
      <div className={right? "chart-info-right-Select": "chart-info-right-text"} >Right</div>
        <div className="chart-info-center">
            <div  className="chart-info-center-top">
            <span className={upper ? "chart-info-center-top-Select-text": "chart-info-center-top-text"}>UPPER JAW</span>
            <span className={Upincisor ? "chart-info-center-top-Select-text": "chart-info-center-top-text"}>incisore</span>
            <span className={Upcanine ? "chart-info-center-top-Select-text": "chart-info-center-top-text"}>canine</span>
            <span className={Uppremolar ? "chart-info-center-top-Select-text": "chart-info-center-top-text"}>pre molare</span>
            <span className={Upmolar ? "chart-info-center-top-Select-text": "chart-info-center-top-text"}>molars</span>
            </div>
            <div  className="chart-info-center-center">
                <button disabled={submitDisbale}> <NavLink className={submitDisbale? "chart-info-center-center-disable chart-info-center-center-btn" : "chart-info-center-center-btn"} to="/selected">Submit</NavLink></button>
            </div>
            <div  className="chart-info-center-bottom">
            <span className={Loincisor ? "chart-info-center-top-Select-text": "chart-info-center-top-text"}>incisore</span>
            <span className={Locanine ? "chart-info-center-top-Select-text": "chart-info-center-top-text"}>canine</span>
            <span className={Lopremolar ? "chart-info-center-top-Select-text": "chart-info-center-top-text"}>pre molare</span>
            <span className={Lomolar? "chart-info-center-top-Select-text": "chart-info-center-top-text"}>molars</span>
            <span className={lower ? "chart-info-center-top-Select-text": "chart-info-center-top-text"}>LOWER JAW</span>
            </div>
        </div>
        <div className={left? "chart-info-left-Select": "chart-info-left-text"}>Left</div>
        
      </div>
      <div className="chart-tooth">
        <svg version="1.1" x="0px" y="0px" viewBox="0 0 450 700">
          <g id="Spots">
            {selectors.reverse().map((e, el) => {
              const idname = selectedTeath[el]
                ? `Tooth${el} selected-${el}`
                : `Tooth${el}`;
              if (el === 2 || el === 6 || el === 18 || el === 23 || el === 27) {
                return (
                  <path
                    className={idname}
                    id={idname}
